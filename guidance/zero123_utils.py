...
class Zero123(nn.Module):
    def __init__(self, device, fp16=True, t_range=[0.2, 0.6], zero123_path='ashawkey/stable-zero123-diffusers'):
        super().__init__()
        ...
        # embeddings = None
        self.use_stable_zero123 = 'stable' in zero123_path

    def get_cam_embeddings(self, polar, azimuth, radius):
        if self.use_stable_zero123:
            T = np.stack([np.deg2rad(polar), np.sin(np.deg2rad(azimuth)), np.cos(np.deg2rad(azimuth)), np.deg2rad(np.full_like(polar, 90))], axis=-1)
            # 90 because pose0 is fixed
            # https://github.com/threestudio-project/threestudio/pull/356/files#diff-7cab41ca8761951def6987763141c5cfe7b1e3c0d174ac3cb0f5b4ca8ec8309aR220
        else:
        ...
        T = torch.from_numpy(T).unsqueeze(0).unsqueeze(0).to(dtype=self.dtype, device=self.device) # [8, 1, 4]
        # print(T.shape)
