...
class ImageDream(nn.Module):
    ...
    def get_image_text_embeds(self, image, prompts, negative_prompts):
        ...
        image_embeddings = {}
        ww = self.model.get_learned_image_conditioning(image_pil).repeat(5,1,1)
        image_embeddings['pos'] = ww # [5, 257, 1280]
        image_embeddings['neg'] = torch.zeros_like(ww)

        image_embeddings['ip_img'] = self.encode_imgs(image)
        image_embeddings['neg_ip_img'] = torch.zeros_like(image_embeddings['ip_img'])

        pos_embeds = self.encode_text(prompts).repeat(5,1,1)
        neg_embeds = self.encode_text(negative_prompts).repeat(5,1,1)
        embeddings = {}
        embeddings['pos'] = pos_embeds
        embeddings['neg'] = neg_embeds
        return image_embeddings, embeddings

    @torch.no_grad()
    def prepare_embeds(self, image_li, prompts, negative_prompts):
        return [self.get_image_text_embeds(image_li[idx:idx + 1], prompts, negative_prompts) for idx in range(len(image_li))]
        # return [self.get_image_text_embeds(x, prompts, negative_prompts) for x in image_li]
    ...
    def train_step(
        ...
        image_embeddings_cur, embeddings_cur = cur_embedding
        batch_size = pred_rgb.shape[0]
        real_batch_size = batch_size // 4
        ...
        camera = camera.repeat(2, 1)
        embeddings = torch.cat([embeddings_cur['neg'].repeat(real_batch_size, 1, 1), embeddings_cur['pos'].repeat(real_batch_size, 1, 1)], dim=0)
        image_embeddings = torch.cat([image_embeddings_cur['neg'].repeat(real_batch_size, 1, 1), image_embeddings_cur['pos'].repeat(real_batch_size, 1, 1)], dim=0)
        ip_img_embeddings= torch.cat([image_embeddings_cur['neg_ip_img'].repeat(real_batch_size, 1, 1, 1), image_embeddings_cur['ip_img'].repeat(real_batch_size, 1, 1, 1)], dim=0)
