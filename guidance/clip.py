...
class CLIP(nn.Module):
    def __init__(self, device, clip_name = 'openai/clip-vit-base-patch32'):
        super().__init__()
    ...
    def get_img_embeds(self, img):
        ...
        # print(image_z.shape, 'clip image embed')
        return image_z
    ...
    def img_img_loss(self, gt_rgb, pred_rgb):
        # pred_rgb = self.aug(pred_rgb)
        pred_rgb = self.resize(pred_rgb)
        pred_rgb = self.normalize(pred_rgb)
...
if __name__ == '__main__':
    clip = CLIP('cuda')
    im = torch.randn((1, 3, 512, 512)).cuda()
    res = clip.get_img_embeds(im)
    print(res.shape)
